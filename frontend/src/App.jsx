...
`.trim()
  ...
  const handleGenerate = async () => {
    ...
    // Always try to persist to server first (teammate's backend)
    const saved = await saveToServer(md)
    if (saved) setSavedToServer(true)

    if (!apiKey.trim()) {
      setOutput(md)
      setShowModal(true)
      showToast(saved
        ? '✓ Saved to server as akita-prompt.md — teammate can now run the agent'
        : 'No API key — showing local markdown (set SAVE_ENDPOINT to enable server save)')
      return
    }
    ...
    try {
      ...
      const resultText = data.content?.[0]?.text || md

      // Save expanded spec to server too
      await saveToServer(resultText)

      setOutput(resultText)
      setShowModal(true)
      showToast(saved ? '✓ Claude expanded + saved to server' : '✓ Generated by Claude')
    } catch (err) {
