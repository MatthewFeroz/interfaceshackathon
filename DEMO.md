...
4. **You see the page build up live.** Claude sends partial progress as it works, so sections appear one by one in the preview. When it's done, the final version locks in.

5. **You click a section to edit it.** The preview is interactive — click any section and a little editor pops up right there. Type "make the headline bigger" or "add a phone number" and Claude revises just that section, leaving everything else untouched.
...
- **Section-level editing.** Click any section in the preview to revise just that part. The AI scopes its changes to that one section.
...
5. Click Generate. Watch Claude work in the terminal. Sections appear one by one in the preview.
6. Click on the hero section in the preview. Type "make it more dramatic with a gradient."
7. Watch Claude revise just the hero — everything else stays the same.
